...
for LOCATION in $BCCF_DB_CREATE_USER_LOCATIONS; do
    LOCATIONS_ARGS="$LOCATIONS_ARGS -l $LOCATION"
done

PLACEHOLDERS_ARGS=""
if [ -n "$BCCF_DB_CREATE_USER_PLACEHOLDERS" ]; then
    IFS="$PLACEHOLDERS_SEPARATOR"
    for PAIR in $BCCF_DB_CREATE_USER_PLACEHOLDERS; do
...
/app/evolve_${EVOLVE_VERSION}_Linux-64bit/evolve $COMMAND $DBMS -c "${BCCF_DB_CREATE_USER_CONNECTION_STRING}" $LOCATIONS_ARGS $SCHEMA_ARGS $PLACEHOLDERS_ARGS $METADATA_TABLE_ARG
